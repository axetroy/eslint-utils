...
function* replace(sourceCode, fixer, identifier, newVariableName) {
    ...
    if (isExportedVariable(identifier)) {
        ...
        let newDeclaration = getText(sourceCode, variableDeclaration)
        ...
        yield* replaceText(fixer, sourceCode, exportNamedDeclaration, newDeclaration)
        ...
        yield* replaceText(fixer, sourceCode, identifier, newVariableName)
        ...
        if (identifier.typeAnnotation) {
            const originalText = getText(sourceCode, identifier)
    ...
    for (const reference of references) {
        if (reference.identifier !== identifier) {
            ...
            if (parent && parent.type === 'Property' && parent.value === reference.identifier && parent.shorthand) {
                ...
                yield* replaceText(fixer, sourceCode, reference.identifier, newVariableName)
            }
        }
    }
}
