        ...
        case 'Identifier': {
            ...
            return variable.defs.some(definition => {
                if (definition.type === 'Variable') {
                    return isBooleanExpression(sourceCode, definition.node.init, depth + 1) || isBooleanTypeAnnotation(definition.node.id.typeAnnotation)
                }
...
function isFunctionReturnBoolean(sourceCode, node) {
    return isBooleanTypeAnnotation(node.returnType) || isBooleanExpression(sourceCode, node.body)
}
